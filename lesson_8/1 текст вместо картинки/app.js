...
buttons.forEach(button => button.addEventListener('click', event => handleClick(event)))
...
function handleClick(clickedButtonEvent) {
    ...
    const cardNode = clickedButtonEvent.target;

    // 3. создайте литерал объекта со следующими свойствами:
    const card = {
        wrap: cardNode.parentNode, // здесь элемент с классом .product
        img: cardNode.parentNode.querySelector('img'), // здесь картинка внутри .product
        productName: cardNode.parentNode.querySelector('.productName'), // здесь .productName, который внутри .product
...
function showMoreText(card) {
    ...
    let someText = 'Здесь могла быть ваша реклама'
